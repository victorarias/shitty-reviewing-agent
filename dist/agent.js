...
export async function runReview(input) {
    ...
    const agent = new Agent({
        ...
        streamFn: (modelArg, context, options) => streamSimple(modelArg, context, {
            ...options,
    ...
    agent.subscribe((event) => {
        ...
        if (event.type === "message_end" && event.message.role === "assistant") {
            ...
            if (usage) {
                ...
                log(`billing model=${event.message.model} input=${usage.input} output=${usage.output} total=${usage.totalTokens} cost=${cost.total.toFixed(6)}`);
            }
        }
...
function deriveErrorReason(message) {
...
async function withRetries(fn, attempts, shouldRetry) {
    let lastError;
